  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-between px-1.5 pointer-events-none">
        <span className={`text-xs transition-opacity duration-300 ${isLight ? 'opacity-0' : 'opacity-40'}`}>
          ☀️
        </span>
        <span className={`text-xs transition-opacity duration-300 ${isLight ? 'opacity-40' : 'opacity-0'}`}>
          🌙
        </span>
      </div>
    </button>
